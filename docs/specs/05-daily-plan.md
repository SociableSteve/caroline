...
## Placement is not re-planning

The plan is drawn once and read all day, so reading it at 14:00 must not propose the morning.
This reconciles with the non-goal below of not re-planning automatically as the day changes:
placement is not re-planning, because the entries, their ranks, their estimates and the
capacity they were fitted against are all unchanged, and no row is rewritten. What the clock
decides is only where an entry is drawn against the free time the plan already claimed.
Criteria 1 to 5 still hold exactly as before, `capacityMinutes` is still a whole-day figure,
and nothing here touches the verdict.

An entry not yet complete is never drawn before the present moment. A completed entry keeps
whatever earlier time it was placed in, whatever its rank, so rank order and clock order can
differ: a done entry ranked fifth can draw before an unfinished one ranked second. That is
correct, because the rank is a proposal and the clock is a record.
...
Issue #82 drew the day bar and the agenda against the clock as well as against the free intervals,
so a plan read mid-afternoon stops proposing the morning. The following is appended rather than
...
23. Free time that has passed with nothing placed in it is time gone rather than capacity: it is
    never offered to work the plan could not fit, and an entry that no longer fits anywhere ahead is
    reported as having no time rather than given one behind the clock. A day whose window has closed
    places no outstanding work at all.
