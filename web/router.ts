...
export type Route =
  ...
  | { readonly name: 'jobs' }
  /** `id` is the conversation being read, or null for a new one. Spec 07. */
  | { readonly name: 'chat'; readonly id: string | null }
...
export const routeLinks = [
  ...
  { href: '#/chat', label: 'Chat', name: 'chat' },
...
export function parseRoute(hash: string): Route {
  ...
  if (segments[0] === 'chat') {
    const id = segments[1]
    if (id === undefined) return { name: 'chat', id: null }

    // As for a project id: an undecodable segment is an unrecognised route rather than a crash.
    const decoded = decode(id)
    return decoded === null ? { name: 'chat', id: null } : { name: 'chat', id: decoded }
  }
